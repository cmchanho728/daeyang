...
- **제품별 문의(각 제품 스펙 카드의 "이 제품 문의" 영역, `ProductCategoryPage.astro`)는 카카오톡만 사용한다.** 전화·이메일 버튼은 넣지 않는다. 하단 고정 상담바(`MobileContactBar.astro`)·푸터·문의 페이지(`contact/index.astro`)·홈의 전화·이메일 버튼은 이 제한과 무관하게 기존처럼 3채널을 그대로 유지한다.
